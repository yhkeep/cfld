...
# cfg.OPTIMIZER.EPOCHS = 1 训练时候打开，使用测试代码的时候关闭
...
def main(cfg):
    ...
    num_samples = 66  # 设置需要训练的数据样本数量
    train_data = Subset(train_data, range(num_samples))


    train_loader = DataLoader(
        train_data,
        # cfg.INPUT.BATCH_SIZE // accelerator.num_processes // cfg.ACCELERATE.GRADIENT_ACCUMULATION_STEPS,
        6 // accelerator.num_processes // cfg.ACCELERATE.GRADIENT_ACCUMULATION_STEPS,
        shuffle = True,
        drop_last = True,
    ...
    lr = cfg.OPTIMIZER.LR * 6 if cfg.OPTIMIZER.SCALE_LR else cfg.OPTIMIZER.LR
